...
public class TransactionServiceImpl implements TransactionService {
	...
	private Optional<Double> checkManintenanceBalance(Long custId, String accountStatus, String accountType,
			double amount, double maintainBalance) {
	...
	public ApiResponse transferFund(FundRequestDto fundRequestDto, String accountStatus, String accountType) {
		...
		if (checkMinimumBalance.isPresent()) {
			if (checkManintenanceBalance(fundRequestDto.getCustId(), ScrotifyConstant.ACCOUNT_ACTIVE_STATUS,
				...
				Account customerAccount = accountRepository.findByCustomerByAccount(fundRequestDto.getCustId(),
				...
				transaction(fundRequestDto.getAccountNo(), fundRequestDto.getAmount(), response, payeeAccount,
						customerAccount);
			} else {
	...
	private void transaction(Long accountNo, double amount, ApiResponse response, Account payeeAccount,
